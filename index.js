...
function cumlelereDonustur(array, ayraç = ",") {
  let newSentence = array.map((item) => item.join(ayraç));
  return newSentence;
}
...
function emojileriDonustur(msg, emojiler) {
  for (let symbol in emojiler) {
    msg = msg
      .replaceAll(symbol.toLowerCase(), emojiler[symbol])
      .replaceAll(symbol.toUpperCase(), emojiler[symbol]);
  }
  return msg;
}
